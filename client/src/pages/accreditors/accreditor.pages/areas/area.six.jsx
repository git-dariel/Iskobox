...
import { fetchAreaSixFoldersAndFiles } from "@/services/folders/folder.service";
...
const AreaSix = () => {
  const googleDriveLinkPPP = documents_links.areasix_ppp;
  const embedLinkPPP = googleDriveLinkPPP.replace("/view?usp=sharing", "/preview");
  const googleDriveLinkSLF = documents_links.areasix_slf;
  const embedLinkSLF = googleDriveLinkSLF.replace("/view?usp=sharing", "/preview");
  const [areaSixData, setAreaSixData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchAreaSixFoldersAndFiles();
      setAreaSixData(data);
    };
        ...
        <section className="flex w-full justify-center bg-gradient-to-r from-amber-300 to-yellow-50 min-h-[40vh] p-7">
          <div className="flex py-4 md:w-[75%] justify-between gap-5">
            <div className="flex flex-col text-justify">
              <h1 className="md:text-2xl font-bold mb-5">{common.AREASIX_CONTENTS.DESC}</h1>
              <p className="md:text-lg text-sm">{common.AREASIX_CONTENTS.DESC_CONTENT}</p>
            </div>
          </div>
        </section>
                    ...
                    >
                      <header className="border w-full text-center h-full py-10">
                        ...
                        {subfolder.name === "Parameter A" && (
                          <p className="text-sm md">Priorities and Relevance</p>
                        )}
                        ...
                        {subfolder.name === "Parameter D" && (
                          <p className="text-sm md:text-base">
                            Community Involvement and Participation in the Extension Activities
                          </p>
                        )}
                      </header>
                    </div>
