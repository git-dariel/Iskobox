...
import { fetchAreaSevenFoldersAndFiles } from "@/services/folders/folder.service";
...
const AreaSeven = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchAreaSevenFoldersAndFiles();
      setAreaSevenData(data);
    };
        ...
        <section className="flex w-full justify-center bg-gradient-to-r from-amber-300 to-yellow-50 min-h-[40vh] p-7">
          <div className="flex py-4 md:w-[75%] justify-between gap-5">
            <div className="flex flex-col text-justify">
              <h1 className="md:text-2xl font-bold mb-5">{common.AREASEVEN_CONTENTS.DESC}</h1>
              <p className="md:text-lg text-sm">{common.AREASEVEN_CONTENTS.DESC_CONTENT}</p>
            </div>
          </div>
        </section>
                    ...
                    >
                      <header className="border w-full text-center h-full py-10">
                        ...
                        {subfolder.name === "Parameter A" && (
                          <p className="text-sm md:text-base">Administration</p>
                        )}

                        {subfolder.name === "Parameter B" && (
                          <p className="text-sm md:text-base">Administrative Staff</p>
                        )}
                        ...
                        {subfolder.name === "Parameter D" && (
                          <p className="text-sm md:text-base">Services and Utilization</p>
                        )}

                        {subfolder.name === "Parameter E" && (
                          <p className="text-sm md:text-base">Physical Set-up and Facilities</p>
                        )}

                        {subfolder.name === "Parameter F" && (
                          <p className="text-sm md:text-base">Financial Support</p>
                        )}

                        {subfolder.name === "Parameter G" && (
                          <p className="text-sm md:text-base">Linkages</p>
                        )}
                      </header>
                    </div>
                    <div
