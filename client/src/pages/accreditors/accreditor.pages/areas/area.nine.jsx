...
import { fetchAreaNineFoldersAndFiles } from "@/services/folders/folder.service";

const AreaNine = () => {
  const googleDriveLinkPPP = documents_links.areanine_ppp;
  const embedLinkPPP = googleDriveLinkPPP.replace("/view?usp=sharing", "/preview");
  const googleDriveLinkSLF = documents_links.areanine_slf;
  const embedLinkSLF = googleDriveLinkSLF.replace("/view?usp=sharing", "/preview");
  const [areaNineData, setAreaNineData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchAreaNineFoldersAndFiles();
      setAreaNineData(data);
    };
        ...
        <section className="flex w-full justify-center bg-gradient-to-r from-amber-300 to-yellow-50 min-h-[40vh] p-7">
          <div className="flex py-4 md:w-[75%] justify-between gap-5">
            <div className="flex flex-col text-justify">
              <h1 className="md:text-2xl font-bold mb-5">{common.AREANINE_CONTENTS.DESC}</h1>
              <p className="md:text-lg text-sm">{common.AREANINE_CONTENTS.DESC_CONTENT}</p>
            </div>
          </div>
        </section>
                    ...
                    >
                      <header className="border w-full text-center h-full py-10">
                        ...
                        {subfolder.name === "Parameter A" && (
                          <p className="text-sm md:text-base">Laboratories, Shop/Facilities</p>
                        )}

                        {subfolder.name === "Parameter B" && (
                          <p className="text-sm md:text-base">Equipment and Supplies</p>
                        )}

                        {subfolder.name === "Parameter C" && (
                          <p className="text-sm md:text-base">Maintenance</p>
                        )}

                        {subfolder.name === "Parameter D" && (
                          <p className="text-sm md:text-base">Special Provisions</p>
                        )}
                      </header>
                    </div>
                    <div
