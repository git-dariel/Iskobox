...
import { fetchAreaTenFoldersAndFiles } from "@/services/folders/folder.service";
...
const AreaTen = () => {
  const googleDriveLinkPPP = documents_links.areaten_ppp;
  const embedLinkPPP = googleDriveLinkPPP.replace("/view?usp=sharing", "/preview");
  const googleDriveLinkSLF = documents_links.areaten_slf;
  const embedLinkSLF = googleDriveLinkSLF.replace("/view?usp=sharing", "/preview");
  const [areaTenData, setAreaTenData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchAreaTenFoldersAndFiles();
      setAreaTenData(data);
    };
        ...
        <section className="flex w-full justify-center bg-gradient-to-r from-amber-300 to-yellow-50 min-h-[40vh] p-7">
          <div className="flex py-4 md:w-[75%] justify-between gap-5">
            <div className="flex flex-col text-justify">
              <h1 className="md:text-2xl font-bold mb-5">{common.AREATEN_CONTENTS.DESC}</h1>
              <p className="md:text-lg text-sm">{common.AREATEN_CONTENTS.DESC_CONTENT}</p>
            </div>
          </div>
        </section>
                    ...
                    >
                      <header className="border w-full text-center h-full py-10">
                        ...
                        {subfolder.name === "Parameter A" && (
                          <p className="text-sm md:text-base">Organization</p>
                        )}

                        {subfolder.name === "Parameter B" && (
                          <p className="text-sm md:text-base">Academic Administration</p>
                        )}

                        {subfolder.name === "Parameter C" && (
                          <p className="text-sm md:text-base">Student Administration</p>
                        )}

                        {subfolder.name === "Parameter D" && (
                          <p className="text-sm md:text-base">Financial Management</p>
                        )}

                        {subfolder.name === "Parameter E" && (
                          <p className="text-sm md:text-base">Supply Management</p>
                        )}

                        {subfolder.name === "Parameter F" && (
                          <p className="text-sm md:text-base">Records Management</p>
                        )}
