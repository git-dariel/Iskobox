...
const AreaTwo = () => {
  const googleDriveLinkPPP = documents_links.areatwo_ppp;
  const embedLinkPPP = googleDriveLinkPPP.replace("/view?usp=sharing", "/preview");
  const googleDriveLinkSLF = documents_links.areatwo_slf;
  const embedLinkSLF = googleDriveLinkSLF.replace("/view?usp=sharing", "/preview");
  const [areaTwoData, setAreaTwoData] = useState([]);
...
`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchAreaTwoFoldersAndFiles();
      setAreaTwoData(data);
    };
          ...
          <div className="flex py-4 md:w-[75%] justify-between gap-5">
            <div className="flex flex-col text-justify">
              <h1 className="md:text-2xl text-base font-bold mb-5">
                {common.AREATWO_CONTENTS.DESC}
              </h1>
              <p className="md:text-lg text-sm">{common.AREATWO_CONTENTS.DESC_CONTENT}</p>
            </div>
          </div>
        </section>
                    ...
                    >
                      <header className="border w-full text-center h-full py-10">
                        ...
                        {subfolder.name === "Parameter C" && (
                          <p className="text-sm md:text-base">Faculty Adequacy and Loading</p>
                        )}

                        {subfolder.name === "Parameter D" && (
                          <p className="text-sm md:text-base">Rank and Tenure</p>
                        )}

                        {subfolder.name === "Parameter E" && (
                          <p className="text-sm md:text-base">Faculty Development</p>
                        )}
                        ...
                        {subfolder.name === "Parameter H" && (
                          <p className="text-sm md:text-base">Professionalism</p>
                        )}
                      </header>
                    </div>
                    <div
