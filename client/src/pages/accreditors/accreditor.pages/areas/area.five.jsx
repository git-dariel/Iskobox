...
import { fetchAreaFiveFoldersAndFiles } from "@/services/folders/folder.service";
...
const AreaFive = () => {
  const googleDriveLinkPPP = documents_links.areafive_ppp;
  const embedLinkPPP = googleDriveLinkPPP.replace("/view?usp=sharing", "/preview");
  const googleDriveLinkSLF = documents_links.areafive_slf;
  const embedLinkSLF = googleDriveLinkSLF.replace("/view?usp=sharing", "/preview");
  const [areaFiveData, setAreaFiveData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchAreaFiveFoldersAndFiles();
      setAreaFiveData(data);
    };
