...
import { fetchAreaEightFoldersAndFiles } from "@/services/folders/folder.service";
...
const AreaEight = () => {
  const googleDriveLinkPPP = documents_links.areaeight_ppp;
  const embedLinkPPP = googleDriveLinkPPP.replace("/view?usp=sharing", "/preview");
  const googleDriveLinkSLF = documents_links.areaeight_slf;
  const embedLinkSLF = googleDriveLinkSLF.replace("/view?usp=sharing", "/preview");
  const [areaEightData, setAreaEightData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchAreaEightFoldersAndFiles();
      setAreaEightData(data);
    };
        ...
        <section className="flex w-full justify-center bg-gradient-to-r from-amber-300 to-yellow-50 min-h-[40vh] p-7">
          <div className="flex py-4 md:w-[75%] justify-between gap-5">
            <div className="flex flex-col text-justify">
              <h1 className="md:text-2xl font-bold mb-5">{common.AREAEIGHT_CONTENTS.DESC}</h1>
              <p className="md:text-lg text-sm">{common.AREAEIGHT_CONTENTS.DESC_CONTENT}</p>
            </div>
          </div>
        </section>
                    ...
                    >
                      <header className="border w-full text-center h-full py-10">
                        ...
                        {subfolder.name === "Parameter A" && (
                          <p className="text-sm md:text-base">Campus</p>
                        )}

                        {subfolder.name === "Parameter B" && (
                          <p className="text-sm md:text-base">Building</p>
                        )}

                        {subfolder.name === "Parameter C" && (
                          <p className="text-sm md:text-base">Classrooms</p>
                        )}

                        {subfolder.name === "Parameter D" && (
                          <p className="text-sm md:text-base">Office Staff and Function Rooms</p>
                        )}

                        {subfolder.name === "Parameter E" && (
                          <p className="text-sm md:text-base">Assembly and athletic Facilities</p>
                        )}

                        {subfolder.name === "Parameter F" && (
                          <p className="text-sm md:text-base">Medical and Dental Clinic</p>
                        )}

                        {subfolder.name === "Parameter G" && (
                          <p className="text-sm md:text-base">Student Center</p>
                        )}

                        {subfolder.name === "Parameter H" && (
                          <p className="text-sm md:text-base">Food Sevices / Canteen</p>
                        )}

                        {subfolder.name === "Parameter I" && (
                          <p className="text-sm md:text-base">Accreditation Center</p>
                        )}

                        {subfolder.name === "Parameter J" && (
                          <p className="text-sm md:text-base">Housing</p>
                        )}
                      </header>
                    </div>
                    <div
